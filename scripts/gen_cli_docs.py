...
def generate_command_doc(
    ...
    options = [p for p in cmd.params if isinstance(p, click.Option) and p.name not in ("help",)]
...
def generate_index(groups: list[tuple[str, click.Group, str]]) -> str:
    ...
    lines.append("")
    lines.append("Vodoo provides a Typer-based CLI with the following subcommands:")
    lines.append("")
