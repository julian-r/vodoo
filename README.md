...
One release tag versions PyPI, npm, and the Swift Package. GitHub Actions publishes the Python artifact and uses stage-only npm Trusted Publishing to stage the TypeScript artifact; a maintainer must review and approve the staged npm package with 2FA before it becomes public. Swift Package Manager resolves the same repository tag. Breaking changes to generated public APIs require a major version, additions require a minor version, and compatible fixes require a patch version.
